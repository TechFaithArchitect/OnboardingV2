...
// Mock Apex methods
jest.mock(
    '@salesforce/apex/OnboardingAppECCController.getRequiredCredentials',
    () => ({ default: jest.fn() }),
    { virtual: true }
);
jest.mock(
    '@salesforce/apex/OnboardingAppECCController.getAvailableCredentialTypes',
    () => ({ default: jest.fn() }),
    { virtual: true }
);
jest.mock(
    '@salesforce/apex/OnboardingAppECCController.createCredentialType',
    () => ({ default: jest.fn() }),
    { virtual: true }
);
jest.mock(
    '@salesforce/apex/OnboardingAppECCController.linkCredentialTypeToRequiredCredential',
    () => ({ default: jest.fn() }),
    { virtual: true }
);
...
describe('c-onboarding-app-vendor-program-ecc-manager', () => {
    ...
    it('renders component with recordId', () => {
        const element = createElement('c-onboarding-app-vendor-program-ecc-manager', {
    ...
    it('shows modal for creating credential type', async () => {
        ...
        const button = element.shadowRoot?.querySelector('lightning-button[data-id="create-type"]');
        if (button) {
            button.click();
            await Promise.resolve();
    ...
    it('handles show modal action', async () => {
        ...
        const showButton = element.shadowRoot?.querySelector('lightning-button[label="Create New Credential Type"]');
        if (showButton) {
            showButton.click();
            await Promise.resolve();
            await Promise.resolve();
    ...
    it('handles close modal action', async () => {
        ...
        // Test close modal
        const closeButton = element.shadowRoot?.querySelector('lightning-button[label="Cancel"]');
        if (closeButton) {
            closeButton.click();
            await Promise.resolve();
        }
    ...
    it('handles name input change', async () => {
        ...
        const nameInput = element.shadowRoot?.querySelector('lightning-input[label="Credential Type Name"]');
        if (nameInput) {
            nameInput.value = 'New Credential Type';
            nameInput.dispatchEvent(new CustomEvent('change', { bubbles: true, composed: true }));
            await Promise.resolve();
        }
    ...
    it('handles link credential error', async () => {
        getRequiredCredentials.mockResolvedValue(mockCredentials);
        getAvailableCredentialTypes.mockResolvedValue(mockCredentialTypes);
        linkCredentialTypeToRequiredCredential.mockRejectedValue(new Error('Link failed'));
