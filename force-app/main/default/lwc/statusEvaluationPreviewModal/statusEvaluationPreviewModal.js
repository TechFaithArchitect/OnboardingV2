...
export default class StatusEvaluationPreviewModal extends LightningElement {
    ...
    wiredPreview({ error, data }) {
        if (data !== undefined) {
            ...
            this.isLoading = false;
            this.showToast('Error', extractErrorMessage(error, 'Failed to load evaluation preview.'), 'error');
            this.traceData = [];
            this.filteredTraceData = [];
        }
    }
    ...
    renderedCallback() {
        if (this.isOpen && this.onboardingId && !this.isLoading && this.traceData.length === 0) {
            this.isLoading = true;
        }
    }
    ...
    applyFilters() {
        ...
        if (this.filters.engineName) {
            ...
            filtered = filtered.filter(trace => 
                trace.engineName && trace.engineName.toLowerCase().includes(engineFilter)
            );
        }
    ...
    handleExportCSV() {
        ...
        // Build CSV content
        const headers = ['Order', 'Group', 'Engine', 'Rule #', 'Requirement', 'Expected Status', 'Passed', 'Evaluation Logic', 'Resulting Status', 'Reason'];
