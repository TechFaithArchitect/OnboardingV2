...
export default class VendorOnboardingPicker extends LightningElement {
    ...
    @api selectedVendorId;
    @api selectedVendorName;
    @api selectedRetailOption;
    @api selectedVertical;
    ...
    handleVendorChange(event) {
        ...
        this.selectedVendorRecord = this.vendorData.find(v => v.vendorId === this.selectedVendorId);
        this.selectedVendorName = this.selectedVendorRecord?.vendorName;

        this.retailOptions = (this.selectedVendorRecord?.availableRetailOptions || []).map(ro => ({
            label: ro,
            value: ro
        }));
    ...
    handleRetailChange(event) {
        ...
        this.verticalOptions = (selectedGroup?.businessVerticals || []).map(v => ({
            label: v,
            value: v
        }));
    ...
    dispatchToFlow() {
        this.dispatchEvent(new CustomEvent('flowvalueselected', {
            detail: {
