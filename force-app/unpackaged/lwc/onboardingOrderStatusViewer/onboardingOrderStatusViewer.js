...
export default class OnboardingOrderStatusViewer extends LightningElement {
    ...
    columns = [
        ...
        { label: 'Effective Date', fieldName: 'effectiveDateFormatted', type: 'text' },
        { label: 'Activated Date', fieldName: 'activatedDateFormatted', type: 'text' }
    ];
        ...
        if (data) {
            this.orders = data.map(order => {
                ...
                if (order.effectiveDate) {
                    row.effectiveDateFormatted = new Date(order.effectiveDate).toLocaleDateString();
                }

                if (order.activatedDate) {
                    row.activatedDateFormatted = new Date(order.activatedDate).toLocaleDateString();
                }
