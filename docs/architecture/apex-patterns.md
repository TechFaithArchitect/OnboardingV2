...
- **Pattern**: `*Service.cls`
- **Location**: `services/`
- **Example**: `OnboardingApplicationService`, `OnboardingRulesService`
...
- **Example**: `OnboardingAppActivationOrchestrator`
...
public with sharing class OnboardingAppECCService {
    ...
    public static External_Contact_Credential_Type__c createCredentialType(String name) {
        ...
        // Delegate to repository for data access
        return OnboardingAppECCRepository.insertCredentialType(name);
    }
}
```
...
public with sharing class OnboardingAppECCRepository {
    ...
    public static External_Contact_Credential_Type__c insertCredentialType(String name) {
        External_Contact_Credential_Type__c newType = new External_Contact_Credential_Type__c(Name = name);
        insert newType;
        return newType;
    }
}
...
### Using Strategies
Orchestrators and services use factories to get strategies:
...
public with sharing class OnboardingAppActivationOrchestrator {
    public static void activate(Id recordId, String objectApiName) {
        ...
        ActivationStrategy strategy = ActivationStrategyFactory.getStrategy(objectApiName);
...
public class OnboardingAppRuleRegistry {
    public static Map<String, List<OnboardingAppValidationRule>> getRules() {
        ...
        rules.put('Vendor_Program_Recipient_Group__c', new List<OnboardingAppValidationRule>{
...
2. ✅ **Services delegate to repositories** for all data access
3. ✅ **Controllers delegate to services** for all business logic
4. ✅ **Orchestrators coordinate** multiple services
5. ✅ **Repositories handle** all SOQL and DML
6. ✅ **Strategies encapsulate** varying behaviors (follows OCP)
7. ✅ **Classes organized** in appropriate subdirectories
8. ✅ **Naming conventions** followed consistently
9. ✅ **Error handling** with meaningful messages
10. ✅ **Input validation** at controller/service boundaries
11. ✅ **Test classes** co-located with source classes
